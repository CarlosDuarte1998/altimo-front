<template>
    ...
    <section class="py-16">
        <div class="container px-4 md:px-6 max-w-[1300px] mx-auto">
            ...
            <div v-else class="grid gap-8 sm:grid-cols-2 lg:grid-cols-4">
                <div 
                    v-for="medico in (medicosStore.medicosByName as Medico[])" 
                    :key="medico.id"
                    class="text-center group cursor-pointer transition-transform duration-300 hover:scale-105"
                >
                    <!-- Imagen del Médico -->
                    <div class="mx-auto mb-4 h-40 w-40 overflow-hidden rounded-full ring-4 ring-gray-100 group-hover:ring-blue-200 transition-all duration-300">
                        <img 
                            :alt="medico.nombre" 
                            loading="lazy" 
                            width="160" 
    ...
    <UModal 
        ...
        :description="selectedMedico ? selectedMedico.especialidad : ''"
        :ui="{
            content: 'w-[calc(100vw-2rem)] max-w-3xl max-h-[calc(100dvh-2rem)] sm:max-h-[calc(100dvh-4rem)] overflow-hidden',
    ...
    >
        <template #body="{ close }">
            <div v-if="selectedMedico" class="space-y-6 pr-2">
                ...
                <div class="flex flex-col sm:flex-row gap-6 items-center sm:items-start">
                    <div class="flex-shrink-0">
                        <div class="h-32 w-32 sm:h-40 sm:w-40 overflow-hidden rounded-full ring-4 ring-blue-100">
                            <img 
                                :alt="selectedMedico.nombre" 
                                class="h-full w-full object-cover"
                                :src="selectedMedico.imagen"
                                @error="handleImageError"
                            >
                        </div>
                    </div>
                    ...
                    <div class="flex-1 text-center sm:text-left">
                        ...
                        <div v-if="selectedMedico.telefono || selectedMedico.email" class="mt-4 space-y-2">
                            <div v-if="selectedMedico.telefono" class="flex items-center justify-center sm:justify-start gap-2 text-sm text-gray-600">
                ...
                <!-- Descripción -->
                <div v-if="selectedMedico.descripcion" class="border-t pt-4">
                    <h4 class="text-lg font-semibold text-gray-900 mb-3">Acerca de</h4>
                ...
                <!-- Certificaciones -->
                <div v-if="selectedMedico.certificaciones && selectedMedico.certificaciones.length > 0" class="border-t pt-4">
                    <h4 class="text-lg font-semibold text-gray-900 mb-2 flex items-center gap-2">
